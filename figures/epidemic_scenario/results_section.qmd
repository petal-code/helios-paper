...
flu_pct_reduction_no_intervention_4 <- results_summary |>
  filter(
    archetype == "flu",
...
covid_pct_reduction_no_intervention_4 <- results_summary |>
  filter(
...
We varied intervention efficacy and population coverage, and considered random and targeted deployment strategies (Figure 4).
...
For the influenza archetype at 60% efficacy, the random strategy required 60% coverage to reach a peak daily incidence of `r signif(flu_random_6$mean_value, 3)` per 1,000, while the targeted strategy achieved the same result with only 40% coverage, producing a peak daily incidence of `r signif(flu_targeted_4$mean_value, 3)` per 1,000 (4G).
This pattern also held at lower coverage levels: the random strategy's result at 40% coverage -- a peak daily incidence of `r signif(flu_random_4$mean_value, 3)` per 1,000 -- matched the targeted strategy's outcome at just 20% coverage, where peak daily incidence was `r signif(flu_targeted_2$mean_value, 3)` per 1,000 (4G).
...
Without intervention, the peak incidence of the influenza archetype was `r signif(filter(no_intervention_flu, metric == "peak_daily_incidence")$mean_value, 3)` per 1,000 (range `r signif(filter(no_intervention_flu, metric == "peak_daily_incidence")$min_value, 3)`-`r signif(filter(no_intervention_flu, metric == "peak_daily_incidence")$max_value, 3)`) with a final epidemic size of `r 100 * signif(filter(no_intervention_flu, metric == "epidemic_final_size")$mean_value, 3)`% of total population (range `r 100 * signif(filter(no_intervention_flu, metric == "epidemic_final_size")$min_value, 3)`%-`r 100 * signif(filter(no_intervention_flu, metric == "epidemic_final_size")$max_value, 3)`%), whereas the SARS-CoV-2 archetype yielded a peak incidence of `r signif(filter(no_intervention_covid, metric == "peak_daily_incidence")$mean_value, 3)` per 1,000 (range `r signif(filter(no_intervention_covid, metric == "peak_daily_incidence")$min_value, 3)`-`r signif(filter(no_intervention_covid, metric == "peak_daily_incidence")$max_value, 3)`) and a final epidemic size of `r 100 * signif(filter(no_intervention_covid, metric == "epidemic_final_size")$mean_value, 3)`% of total population (range `r 100 * signif(filter(no_intervention_covid, metric == "epidemic_final_size")$min_value, 3)`%-`r 100 * signif(filter(no_intervention_covid, metric == "epidemic_final_size")$max_value, 3)`%).
...
Random AQI installation at 40% coverage and 60% efficacy reduced the final epidemic size by `r signif(100 * flu_pct_reduction_no_intervention_4$mean_pct_reduction, 3)`% (range `r signif(100 * flu_pct_reduction_no_intervention_4$min_pct_reduction, 3)`%-`r signif(100 * flu_pct_reduction_no_intervention_4$max_pct_reduction, 3)`%), compared with only `r signif(100 * covid_pct_reduction_no_intervention_4$mean_pct_reduction, 2)`% (range `r signif(100 * covid_pct_reduction_no_intervention_4$min_pct_reduction, 2)`%-`r signif(100 * covid_pct_reduction_no_intervention_4$max_pct_reduction, 2)`%) for the SARS-CoV-2 archetype under the same settings (Figure 4A & 4E).

Increasing either efficacy or coverage yielded similar increases in the impact on epidemic peak, timing and final size.
For the influenza archetype, expanding randomly allocated coverage from 40% to 60% at 40% efficacy lowered the final epidemic size by `r signif(100 * pct_reduction("flu", "epidemic_final_size", "Random", 0.4, 0.6, 0.4, 0.4), 3)`%, approximately matching the `r signif(100 * pct_reduction("flu", "epidemic_final_size", "Random", 0.6, 0.4, 0.4, 0.4), 3)`% from instead increasing efficacy from 40% to 60% while holding coverage at 40% (Figure 4E).
...
For the influenza archetype at 60% efficacy, the random strategy required 60% coverage to reach an average peak daily incidence of `r signif(flu_random_6$mean_value, 3)` per 1,000, while the targeted strategy achieved the same result with only 40% coverage, producing an average peak daily incidence of `r signif(flu_targeted_4$mean_value, 3)` per 1,000 (Figure 4G).
This pattern also held at lower coverage levels: the random strategy’s result at 40% coverage (average peak daily incidence of `r signif(flu_random_4$mean_value, 3)` per 1,000) matched the targeted strategy’s outcome at just 20% coverage, where peak daily incidence was `r signif(flu_targeted_2$mean_value, 3)` per 1,000 (Figure 4G).
