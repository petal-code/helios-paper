...
update_labels <- function(df) {
  df |>
    mutate(
      ...
      coverage_fct = factor(
        pct(coverage),
        levels = pct(sort(unique(coverage)))
      ),
      efficacy_fct = factor(
        pct(efficacy),
        levels = pct(sort(unique(efficacy)))
      )
    )
}
...
pointrange_plot_metric <- function(metric, archetype) {
  ...
  ggplot(df, aes(x = coverage_fct, col = efficacy_fct)) +
    geom_line(aes(y = mean_value, group = efficacy_fct)) +
    geom_pointrange(
      aes(
        y = mean_value,
        ymin = min_value,
...
plot <- row1 / row2 / row3 / row4 + plot_annotation(tag_levels = "A")
