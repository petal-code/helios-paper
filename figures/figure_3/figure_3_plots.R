...
extract_df <- function(p_list, v_list, label) {
  sw <- generate_far_uvc_switches(
    parameters_list = p_list,
    variables_list = v_list
  )
  data.frame(
    setting_type = c(
      rep("school", length(sw$school_specific_riskiness)),
      rep("leisure", length(sw$leisure_specific_riskiness)),
      rep("workplace", length(sw$workplace_specific_riskiness))
    ),
    riskiness = c(
      sw$school_specific_riskiness,
...
df <- bind_rows(
  extract_df(p_rand, variables, "Random Targeting of AQI"),
  extract_df(p_targ, variables, "Targeting AQI Based on Riskiness")
) |>
  arrange(desc(riskiness)) |>
  mutate(rank = row_number(), installed = ifelse(uvc == 1, "Yes", "No"))

panel_a <- ggplot(
  df,
  aes(x = rank, y = riskiness, fill = installed, colour = installed)
) +
  ...
  labs(
    x = "Rank Order of Riskiness of Locations",
    y = "Relative riskiness",
    fill = "AQI Installed?",
    colour = "AQI Installed?"
  ) +
...
#targeting results
dir_sim <- "helios-paper/figures/figure_3/figure_3_outputs"
...
reductions <- metrics %>%
  ...
  mutate(
    pct_reduction_incidence = (mean_annualized_incidence_baseline -
      mean_annualized_incidence_post) /
...
panel_b_summary <- reductions %>%
  ...
  mutate(
    efficacy_label = paste0(
      "Efficacy = ",
      scales::percent(efficacy, accuracy = 1)
    ),
    coverage_label = scales::percent(coverage, accuracy = 1),
    fill_key = ifelse(
      coverage_type == "random",
      "random",
      paste0("targeted_", efficacy)
    )
  )
...
panel_b <- ggplot(
  ...
  scale_fill_manual(
    values = c(
      "random" = "grey70",
      "targeted_0.4" = "#839bd8", # light blue
      "targeted_0.6" = "#3b5fba", # medium blue
      "targeted_0.8" = "#1d2f5d" # dark blue
    ),
    breaks = c("random", "targeted_0.4", "targeted_0.6", "targeted_0.8"),
    labels = c(
      "Random",
      "Targeted (40%)",
      "Targeted (60%)",
...
panel_c <- ggplot(
  panel_c_summary,
  aes(x = factor(coverage_label), y = mean_rel_impact, fill = factor(efficacy))
) +
  ...
  scale_fill_manual(
    values = c("0.4" = "#839bd8", "0.6" = "#3b5fba", "0.8" = "#1d2f5d"),
    labels = c("40%", "60%", "80%"),
    name = "Efficacy"
  ) +
  scale_y_continuous(
...
panel_d <- ggplot(
  panel_d_summary,
  aes(x = factor(coverage_label), y = mean_diff, fill = factor(efficacy))
) +
  geom_col(position = dodge, width = 0.8) +
  geom_errorbar(
    aes(ymin = low, ymax = high),
...
panel_e <- ggplot(
  ...
  scale_fill_manual(
    values = c(
      "random" = "grey70",
      "targeted_0.4" = "#dc6a8b", # light pink
      "targeted_0.6" = "#c62f5a", # medium pink
      "targeted_0.8" = "#63172d" # dark pink
    ),
    breaks = c("random", "targeted_0.4", "targeted_0.6", "targeted_0.8"),
    labels = c(
      "Random",
...
panel_f <- ggplot(
  ...
  scale_fill_manual(
    values = c("0.4" = "#dc6a8b", "0.6" = "#C62f5a", "0.8" = "#63172d"),
    labels = c("40%", "60%", "80%"),
    name = "Efficacy"
  ) +
  scale_y_continuous(
